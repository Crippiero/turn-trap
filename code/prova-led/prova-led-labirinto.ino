...
void renderNewMaze(int m[][], Point pl1 = {-1, -1}, Point pl2 = {-1, -1}, Point pl3 = {-1, -1}, Point pl4 = {-1, -1}) {
    ...
    for(int i=0; i<ROWS; i++){
      for(int j=0; j<COLS; j++){
          if(m[i][j] == '#'){
              ...
              strip.setPixelColor((i*ROWS)+j, strip.Color(0, 255, 0));
          }   
          else if(m[i][j] == 'G'){
              strip.setPixelColor((i*ROWS)+j, strip.Color(0, 0, 255));
         }         
     }
  }
}
...
void loop() {
    // Esempio con due giocatori
    renderNewMaze({0, 0}, {5, 14});

    delay(2000); 
    
    // Esempio con tre giocatori
    renderNewMaze({1, 1}, {5, 5}, {10, 2});
